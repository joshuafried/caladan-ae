...
constexpr int kFanoutSize = 8;
constexpr int kNumLeafs = 32;
constexpr int kLBPerFanout = 4;
// Upstream Payload
struct payload {
  uint64_t user_id;
  uint64_t movie_id;
...
class ChildQueue {
public:
  ChildQueue(): next_index_(0), num_outst_reqs_(0) {
  ...
  uint64_t GetOutstReqs() {
    return num_outst_reqs_;
  }
...
public:
  ...
  int SelectConnection(int idx) {
    int base_idx = kLBPerFanout*idx;
    int min_idx = base_idx + start_idx_[idx];
    uint64_t min_outst_reqs = child_qs_[min_idx]->GetOutstReqs();
    for (int i = 1; i < kLBPerFanout; ++i) {
      int offset = (start_idx_[idx] + i) % kLBPerFanout;
      if (min_outst_reqs < child_qs_[base_idx + offset]->GetOutstReqs()) {
        min_idx = base_idx + offset;
        min_outst_reqs = child_qs_[min_idx]->GetOutstReqs();
      }
    } 
  ...
  void FanoutAll(FanoutTracker* ft) {
    ...
    for (int i = 0; i < kFanoutSize; ++i) {
      if (bs[i]){
        ...
        child_qs_[idx]->EnqueueRequest(ft);
      }
    }
    PushBack(ft);
  }
...
void FanoutHandler(void *arg) {
  ...
  rt::WaitGroup starter(kNumLeafs + 1);
...
int main(int argc, char *argv[]) {
  ...
  laddrs.reserve(kNumLeafs);
  for (i = 0; i < kNumLeafs; ++i) {
    netaddr laddr;
    ret = StringToAddr(argv[2+i], &laddr.ip);
