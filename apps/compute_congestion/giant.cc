...
constexpr uint64_t kMaxCatchUpUS = 10000;
...
std::vector<work_unit> GiantClientWorker(
...
//      w[idx].cpu = ntoh32(rp.cpu);
  ...
  while (true) {
    ...
    while (true) {
      new_cwnd = floor(duration_cast<seconds>(now - expstart).count());
    ...
    while ((double)(num_outst_req + 1) > cwnd) {
      cv_.Wait(&m_);
    }
...
//    w[i].timing = duration_cast<sec>(timings[i] - expstart).count();
    ...
    p.work_iterations = hton64(1000 * kIterationsPerUS);
    p.index = hton64(index);
...
//    if (duration_cast<sec>(now - expstart).count() > kExperimentTime) break;
    ...
    if (cwnd < 1.0) {
      double time_to_sleep = 100.0 / cwnd;
...
std::vector<work_unit> RunExperiment(
  ...
  rt::WaitGroup starter(threads + 2);
  std::vector<rt::Thread> th;
  std::unique_ptr<std::vector<work_unit>> samples[threads+1];
  std::vector<std::pair<uint64_t, uint64_t>> queues;
  std::vector<std::pair<uint64_t, double>> cwnds;
  ...
  th.emplace_back(rt::Thread([&] {
    auto v = GiantClientWorker(conns[threads].get(), &starter);
    samples[threads].reset(new std::vector<work_unit>(std::move(v)));
  }));
...
// Print aggregated throughput
  ...
  for (const work_unit &u : w) {
    if (u.timing <= next_target) {
      ...
      agt_out << next_target/1000000.0 << "," << num_req_out/double(granularity) << std::endl;
      next_target += granularity;
      while (u.timing > next_target) {
        agt_out << next_target/1000000.0 << ",0" << std::endl;
        next_target += granularity;
      }
      num_req_out = 1;
    }
  ...
  for (auto &q : queues) {
    if (q.first <= next_target) {
      sum_queue += q.second;
      cnt_queue++;
    } else {
      q_out << next_target/1000000.0 << "," << sum_queue/double(cnt_queue) << std::endl;
      next_target += granularity;
      while (q.first > next_target) {
      q_out << next_target/1000000.0 << "," << std::endl;
      next_target += granularity;
    }
...
double Get10us(std::mt19937 &rgen) {
  return 10.0;
}
...
void SteadyStateExperiment(int threads, double offered_rps,
  ...
  std::vector<work_unit> w = RunExperiment(threads, &rps, &cpu_usage, [=] {
    ...
    return GenerateWork(std::bind(rd, rg), std::bind(Get10us, dg), 0, kExperimentTime);
  });
...
#if 1
  for (double i = 700000; i <= 700000; i += 100000) {
