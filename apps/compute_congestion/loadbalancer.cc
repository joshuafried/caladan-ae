...
std::vector<work_unit> ClientWorker(
  ...
  std::vector<double> ewma_delays_;
  ewma_delays_.reserve(num_servers);
  for (int i = 0; i < num_servers; ++i)
    ewma_delays_[i] = 0.0;
  ...
  for (int i = 0; i < num_servers; ++i) {
    th.emplace_back(rt::Thread([&, i] {
      ...
      while (true) {
        ...
        metrics[i] = queueing_delay;

        ewma_delays_[i] = 0.8*ewma_delays_[i] + 0.2*queueing_delay;

        if (ewma_delays_[i] > 150) {
          if (num_outst_reqs[i] <= cwnds_[i])
            new_cwnd = cwnds_[i] * 0.8;
        } else if (ewma_delays_[i] <= 100) {
