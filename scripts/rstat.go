...
func prettyPrint(m, lastm map[string]uint64, interval int) {
	...
	fmt.Printf("net: RX %.1f pkts, %.1f bytes | TX %.1f pkts, %.1f bytes | %.1f drops | %.2f%% rx out of order (%.2f%% reorder time) | %.2f%% TCP migration\n",
		   ...
		   dm["rx_tcp_text_cycles"] / (dm["sched_cycles"] + dm["program_cycles"]) * 100, 100 * dm["tcp_lock_steal"] / dm["tcp_lock"])
