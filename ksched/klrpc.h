/*
 * ksched_lrpc.h - shared memory communication channels
 *
 * Copied from the shenango base library.
 */
...
static inline bool lrpc_send_mwait(struct lrpc_chan_out *chan, uint64_t cmd,
			     unsigned long payload)
{
	struct lrpc_msg *dst;
	...
	if (unlikely(chan->send_head - chan->send_tail >= chan->size)) {
		if (chan->send_head - lrpc_poll_send_tail(chan) >= chan->size) {
			mwait(chan->recv_head_wb, chan->send_tail);
			if (unlikely(chan->send_head - chan->send_tail >= chan->size)) {
				return false;
			}
		}
			     ...
			     unsigned long *payload_out)
{
	...
	*payload_out = m->payload;
	smp_store_release(chan->recv_head_wb, chan->recv_head);
	return true;
}

static inline bool lrpc_recv_mwait(struct lrpc_chan_in *chan, uint64_t *cmd_out,
				   unsigned long *payload_out)
{
	struct lrpc_msg *m = &chan->tbl[chan->recv_head & (chan->size - 1)];
